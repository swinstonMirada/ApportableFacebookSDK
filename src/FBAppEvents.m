...
- (void)handleActivitiesPostCompletion:(NSError *)error
    ...
    if (flushResult == FlushResultServerError) {
        BOOL devError = allEventsAreImplicit == false;
        [FBAppEvents logAndNotify:[error description] allowLogAsDeveloperError:&devError];
    }
...
+ (void)logAndNotify:(NSString *)msg {
    BOOL devError = YES;
    [FBAppEvents logAndNotify:msg allowLogAsDeveloperError:&devError];
}
